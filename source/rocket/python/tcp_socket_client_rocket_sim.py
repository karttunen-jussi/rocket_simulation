...
def GetUserKeyboardCommand():
    while True:
        command = input("Waiting keyboard command...\n")
        power_command_values = command.split(";")
...
def ReceiveDataFromTcpSocket():
    while True:
        ...
        data_converted_tuple = struct.unpack("<2f", data_received_raw)
...
def SendDataToTcpSocket():
    while True:
        command_frame = struct.pack("<48s", "Testing TCP socket connection".encode())
        socket_rocket_sim.sendall(command_frame)
        time.sleep(2.0)

        # if (not command_queue.empty()):
        #     command = command_queue.get()
        #     command_frame = struct.pack("<2f", command[0], command[1])
        #     socket_rocket_sim.sendall(command_frame)
        # else:
        #     time.sleep(0.1)
